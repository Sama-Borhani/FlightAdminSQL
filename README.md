...
--Joining FLIGHTS, CUSTOMERS, and BOOKINGS to get flight booking details along with customer information:
SELECT 
    F.FLIGHTID,
    F.ARRIVALTIME,
    F.DEPARTURETIME,
...
SELECT PROGRAMID, CUSTOMERID, POINTS, STATUS
FROM FREQFLYER
WHERE STATUS = 'Gold';
...
-- Altering the COMPANIES table to add constraints for EMAIL and CONTACTNUMBER
...
ALTER TABLE COMPANIES
ADD CONTACTNUMBER VARCHAR(15) NOT NULL
CONSTRAINT contact_number_format CHECK (CONTACTNUMBER ~ '^[+]?[0-9]{10,15}$');
